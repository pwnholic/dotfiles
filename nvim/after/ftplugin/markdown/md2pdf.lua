...
local function complete_md_to_pdf(arglead, _, _)
	...
	elseif arglead == "--pdf-engine=" then
		return { "latexmk", "lualatex", "pdflatex", "wkhtmltopdf", "xelatex" }
	end
	return {}
end
...
local function md_to_pdf(tbl)
	...
	vim.system({ "md2pdf", "--" .. opts["pdf-engine"], table.unpack(args) }, {}, function(obj)
		if obj.code ~= 0 then
			vim.schedule(function()
				vim.notify(
					string.format("[markdown-md2pdf] md2pdf failed with code %d: %s", obj.code, obj.stderr),
