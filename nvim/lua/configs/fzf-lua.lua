...
M.init = vim.schedule_wrap(function()
	vim.ui.select = function(...)
		...
		if not fzf_ui_select.is_registered() then
			local _ui_select = fzf_ui_select.ui_select
			fzf_ui_select.ui_select = function(items, opts, on_choice)
				opts.prompt = opts.prompt and vim.fn.substitute(opts.prompt, ":\\?\\s*$", ":\xc2\xa0", "")
				_ui_select(items, opts, on_choice)
			end

			fzf_ui_select.register(function(_, items)
				return {
					winopts = {
						split = string.format(
							"botright %dnew",
...
function M.setup()
	...
	function fzf_actions.switch_cwd()
		...
		fzf_actions.resume()
		vim.notify(string.format("switch to %s", opts.cwd), 2)
	end
	...
	function fzf_actions.del_autocmd(selected)
		...
		fzf_lua.autocmds({ fzf_opts = { ["--query"] = vim.fn.shellescape(last_query) } })
	end

	local _diagnostics_document = fzf_lua.diagnostics_document
	...
	local no_preview_opts = {
		...
		lsp = {
			...
			code_actions = { winopts = { split = string.format("botright %d new", (vim.o.lines / 4)) } },
	...
	map("n", "<leader>uz", function()
		...
		if enabled then
			...
			return fzf_lua.setup(no_preview_opts)
		else
			...
			return fzf_lua.setup(vim.tbl_extend("force", no_preview_opts, preview_opts))
		end
	end, { desc = "Toggle FZF Preview" })

	fzf_lua.setup(no_preview_opts)
	...
	local function fzfmap(builtin, opts)
		...
		return function()
			...
			opts = params.opts
			opts =
				vim.tbl_extend("force", { cwd = require("utils.root").get_root(), fzf_opts = fzf_opts_map }, opts or {})
			fzf_lua[builtin](opts)
		end
	end
