    ...
    opts = function()
        ...
        local function add_to_harpoon(selected, opts)
            ...
            for i = 1, #selected do
                ...
                require("harpoon"):list():add({ value = file.bufname or file.path or file.uri, context = { row = pos[1], col = pos[2] } })
        ...
        return {
            winopts = {
                ...
                on_close = function()
                    if vim.g._fzf_leave_win and vim.api.nvim_win_is_valid(vim.g._fzf_leave_win) and vim.api.nvim_get_current_win() ~= vim.g._fzf_leave_win then
...

            {
                ...
                function()
                    require("fzf-lua").files({
                        ...
                        fd_opts = [[--color=never --type d --follow --exclude .git]],
                        rg_opts = [[--color=never --files --follow -g '!.git'"]],
                    })
                end,
                desc = "Directory",
