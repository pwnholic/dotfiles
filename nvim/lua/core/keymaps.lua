local map, toggle = vim.keymap.set, require("utils.toggle")
...
map("n", "<leader>uc", function()
	toggle.toggle_option("conceallevel", false, { 0, conceallevel })
end, { desc = "Toggle Conceal Level" })
map("n", "<leader>um", function()
	toggle.toggle_option("showtabline", false, { 0, showtabline })
...
map({ "n", "x" }, "q", function()
	require("utils.window").close_whith_q()
...
map("n", "[w", diag_goto(false, "WARN"), { desc = "Prev Warn" })
map("n", "]w", diag_goto(true, "WARN"), { desc = "Next Warn" })
map("n", "[i", diag_goto(false, "INFO"), { desc = "Prev Info" })
map("n", "]i", diag_goto(true, "INFO"), { desc = "Next Info" })
map("n", "[h", diag_goto(false, "HINT"), { desc = "Prev Hint" })
map("n", "]h", diag_goto(true, "HINT"), { desc = "Next Hint" })
...
map("n", "cq", [[:\<C-u>call v:lua.SetupMultipleCursors()<CR>*``qz]], { desc = "Inititiate with macros" })
map(
	"x",
	"cq",
