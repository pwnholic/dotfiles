...
local function augroup(group, ...)
	...
	for _, a in ipairs({ ... }) do
		a[2].group = id
		vim.api.nvim_create_autocmd(table.unpack(a))
	end
end
		...
		callback = function(info)
			...
			vim.schedule(function()
				...
				vim.api.nvim_win_call(win, function()
					...
					local target_dir = require("utils.root").get_root() or vim.fs.dirname(info.file)
...
augroup("KeepWinRatio", {
	{ "VimResized", "TabEnter" },
	{
		...
		callback = function()
			vim.cmd.wincmd("=")
			require("utils.window").rest_ratio(vim.api.nvim_tabpage_list_wins(0))
		end,
	},
}, {
	{ "TermOpen", "WinResized", "WinNew" },
	{
		...
		callback = function()
			...
			require("utils.window").save_ratio(vim.v.event.windows)
		end,
	},
})
