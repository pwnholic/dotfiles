...
function M.start(config, opts)
    ...
    local function _validate(dir)
        return dir and dir ~= "." and (vim.uv.fs_stat(dir) or {}).type == "directory" and dir or nil
    end

    local root_dir = _validate(vim.fs.root(bufname, vim.list_extend(config.root_patterns or {}, M.default_config.root_patterns or {}))) or _validate(vim.fs.dirname(bufname))
    if not root_dir then
        return
    end
    return vim.lsp.start(vim.tbl_deep_extend("keep", config or {}, { name = name, root_dir = root_dir }, M.default_config), opts)
end
...
local subcommand_arg_handler = {
    ...
    range = function(args, tbl)
        args.range = args.range or tbl.range > 0 and {
            ["start"] = { tbl.line1, 0 },
            ["end"] = { tbl.line2, 999 },
        } or nil
        return args
    ...
    lsp = {
        ...
        restart = {
            ...
            fn_override = function(ids)
                ...
                local clients = not vim.tbl_isempty(ids) and vim.tbl_map(function(id)
                    return vim.lsp.get_client_by_id(id)
                end, ids) or vim.lsp.get_clients({ bufnr = 0 })
                for _, client in ipairs(clients) do
        ...
        if cmdline:sub(1, cursorpos):match("^%A*" .. meta .. "%s+%S*$") then
            return vim.tbl_filter(
                ...
                vim.tbl_filter(function(key)
                    ...
                    return info and (info.arg_handler or info.params or info.opts or info.fn_override or info.completion) and true or false
                end, vim.tbl_keys(subcommand_info_list))
            )
        end
        -- If subcommand is specified, complete with its options or params
        local subcommand = utils.camel_to_snake(cmdline:match("^%s*" .. meta .. "(%w+)")) or cmdline:match("^%s*" .. meta .. "%s+(%S+)")
