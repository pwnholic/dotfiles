...
M.theme_path = lazy_util.norm(vim.fn.stdpath("cache") .. "/lazygit-theme.yml")
...
function M.open(opts)
	if vim.g.lazygit_theme ~= nil then
		lazy_util.deprecate("vim.g.lazygit_theme", "vim.g.lazygit_config")
	end
	...
	if vim.g.lazygit_config then
		...
		if not M.config_dir then
			...
			if ok then
				...
				vim.env.LG_CONFIG_FILE = lazy_util.norm(M.config_dir .. "/config.yml" .. "," .. M.theme_path)
			else
	...
	return utils.terminal(cmd, opts)
end
...
function M.get_color(v)
	...
	local color = {}
	if v.fg then
		color[1] = utils.get_hexcolor(v.fg)
	elseif v.bg then
		color[1] = utils.get_hexcolor(v.bg, true)
	end
	if v.bold then
...
gui:
  ...
  theme:
	...
	lazy_util.write_file(M.theme_path, config)
	M.dirty = false
end
...
function M.blame_line(opts)
	...
	local root = lazy_util.root.detectors.pattern(0, { ".git" })[1]
	local cmd = { "git", "-C", root, "log", "-n", opts.count, "-u", "-L", line .. ",+1:" .. file }
	return lazy_util.float_cmd(cmd, opts)
end

function M.browse()
	local config = require("lazy.manage.git").get_config(lazy_util.root.detectors.pattern(0, { ".git" })[1])
	...
	local function open(remote)
		if remote then
			lazy_util.info(("Opening [%s](%s)"):format(remote.name, remote.url))
			vim.ui.open(remote.url)
		end
	end

	if #remotes == 0 then
		return lazy_util.error("No git remotes found")
