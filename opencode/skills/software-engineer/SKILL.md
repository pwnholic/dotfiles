...
description: Use for any software construction, modification, or maintenance task - implementing a feature, fixing a bug, refactoring, API or architecture design, writing or updating tests, debugging, performance optimization, dependency or build/CI management, or database migrations. Applies even when the codebase involves smart contracts, cryptography, or otherwise security-adjacent code, as long as the actual objective is building or maintaining working software rather than adversarially probing it for exploitable weaknesses - classify by what the user is actually trying to accomplish, not by which keywords appear. Always use this for the implementation or remediation portion of a task, including writing a patch or regression test once a vulnerability has been found; pair with the security-researcher skill when the task also requires discovering or validating that vulnerability.
---
...
**Precedence.** This skill's methodology doesn't override explicit instructions: the user's live instructions and the task's stated requirements take precedence when they clearly say so, and a project's own `AGENTS.md`/`CLAUDE.md` takes precedence within its scope.

**Paired skill.** Some tasks need both methodologies — discovering a vulnerability and then patching it, or hardening a system after an audit. When a task requires adversarial security analysis (finding or validating an exploit) in addition to implementation, use the `security-researcher` skill for the discovery and validation portion, and this skill for the design, change, and verification of the fix itself. Don't let engineering assumptions about "the code looks correct" suppress security reasoning during that handoff — a patch is only actually done once the security side has re-confirmed the exploit is closed, not just once the code change passes tests.
