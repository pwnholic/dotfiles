...
description: Use for any task whose primary objective is attacker-oriented - discovering, analyzing, validating, or reproducing a security vulnerability or exploit. Covers vulnerability research, bug bounty work, smart-contract or DeFi/protocol security review, attack-surface and trust-boundary analysis, authorization or authentication bypass, oracle manipulation, replay or transaction-ordering attacks, economic/MEV attacks, invariant violations, and determining whether an already-audited, heavily hardened system is actually exploitable. Use this even when a task is phrased as ordinary code review or debugging if the real goal is adversarial - classify by actual objective, not by keywords like "security" or "audit" alone. Pair with the software-engineer skill when the task also requires implementing the patch or regression test once a vulnerability is confirmed.
---
...
Every serious question in this skill has the same shape:
...
**Scope.** This skill governs analytical reasoning. It does not authorize action against any system beyond what the current engagement has explicitly granted — that boundary is defined in §12 and applies regardless of how aggressive the analysis in the rest of this skill gets.

**Paired skill.** A common lifecycle is discover → validate → root-cause → patch → regression-test → re-confirm. For the discovery, validation, and root-cause portion, this skill is authoritative — prioritize attacker capabilities, trust boundaries, invariants, and exploitability over engineering concerns. Once the fix itself is being designed and implemented, hand off to the `software-engineer` skill for change discipline and verification, then return here to re-assess exploitability and confirm the mitigation actually closes the condition rather than just hiding the symptom.
